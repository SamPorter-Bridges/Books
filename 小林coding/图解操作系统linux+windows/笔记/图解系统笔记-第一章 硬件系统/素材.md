...
32 位 CPU 配 40 位地址总线之所以实现起来比较复杂，本质原因是：
...
第二，指令格式无法直接编码 40 位地址。

CPU 指令长度通常有限，例如：

```
32bit / 64bit instruction
```
...
# 所以32位也无法匹配64位 原因比40位更加极端

严格来说，**32 位 CPU 无法原生匹配 64 位地址空间**，原因和 32→40 位一样，但更极端。核心问题依然是 **CPU 内部寄存器宽度限制**：

1. **寄存器无法存下完整地址**
   - 32 位 CPU 的通用寄存器只有 32 bit，最大能存 2³² = 4GB 地址。
   - 64 bit 地址需要 64 bit 寄存器才能直接存储。
   - 32 bit CPU 只能通过 **寄存器对 + 软件/硬件扩展机制** 来访问超过 4GB 的地址，这会极大增加复杂度。
2. **指令编码问题**
   - 32 位指令格式只支持 32 bit 地址。
   - 要访问 64 bit 地址，必须用额外寄存器或者分段/扩展机制，指令会变得复杂且慢。
3. **地址生成单元（AGU）和总线**
   - 32 bit CPU 内部 AGU 只能做 32 bit 加法。
   - 访问 64 bit 地址需要硬件额外扩展，甚至可能分多条 micro-op 执行。
4. **操作系统和软件兼容性**
   - 32 bit 指针类型仍然只有 32 bit。
   - 要访问 64 bit 地址空间，操作系统必须引入 “window mapping” 或 “bank switching” 等机制，程序无法直接使用 64 bit 指针。
5. **TLB / Cache / 页表扩展复杂**
   - 页表项需要更大，TLB也要扩展。
   - 32 bit CPU 的原生设计难以支持直接映射整个 64 bit 地址空间。

------

所以结论：

- **32 bit CPU 可以通过硬件扩展（类似 PAE）访问 >4GB 内存，但远不能原生支持 64 bit 地址空间**
- 要原生支持 64 bit 地址，需要 **CPU 寄存器、指令集、地址生成、页表、操作系统指针全部升级** → 这就是 64 位 CPU 的必要性。

简单理解：

> 32 bit CPU 最大可直接寻址 4GB，想访问 64 bit 地址空间只能靠绕路，复杂且效率低。
...
**1）数据竞争（Data Race）**
这是一个**正确性问题**。在内存模型（例如 C++ 内存模型）里，如果：

- 多个线程同时访问同一内存位置
- 至少一个是写
- 且没有同步（锁、原子、内存序）

就会产生未定义行为（UB）。这和 Cache、CPU 无关，是语言层面的语义问题。

------

**2）伪共享（False Sharing）**
这是一个**性能问题**。本质是 CPU cache coherence（缓存一致性协议，例如 MESI）导致的：

- 不同线程修改**不同变量**
- 但这些变量恰好落在**同一个 cache line（通常 64B）**
- 一个线程写入 → 该 cache line 被标记为 Modified
- 其他 CPU 核心上的同一 cache line 副本失效（Invalid）
- 导致频繁的 cache line bouncing（缓存行来回失效/转移）

结果是：性能急剧下降，但**逻辑上仍然是正确的**。
...
> 数据竞争破坏正确性，伪共享破坏性能，它们不是因果关系，而是两个不同层面的并发问题。

# 为什么CPU缓存一致性保证数据不会错乱

这个问题本质涉及 **CPU缓存一致性协议（cache coherence）+ 内存模型**。结论先说清楚：

> **不会读到“物理错误数据”，但可能读到“逻辑上不符合预期的数据”**。

也就是说：

- CPU 不会读到“随机垃圾值”
- 但在没有同步的情况下，读到的值**不保证符合你想要的顺序或次数**

下面拆开讲。

------

# 一、两个CPU同时修改同一个变量时发生了什么

假设两个核心（Core0 / Core1）同时执行：
...
$$
a++ \equiv \text{load} \rightarrow \text{add} \rightarrow \text{store}
$$
...
> **同一时刻，一个 cache line 只能有一个核心拥有“写权限（Modified）”**
...
1. 它必须先拿到 cache line 的 **独占权限**
2. 会发送 **Invalidate（失效消息）**
3. 其他核心（Core1）把该 cache line 标记为 I（无效）

👉 所以：

> **不会出现两个核心同时在写同一个cache line的有效副本**
...
- 如果自己的 cache line 是 Invalid
- 会从：
  - 内存
  - 或者 Core0（如果它是 Modified）

拿到**最新值**

👉 这叫：

> **写传播（write propagation） + 缓存一致性**
...
> CPU cache coherence 保证同一时刻只有一个写者，读要么读旧值，要么读新值，不会读垃圾
...
> 写入会通过 cache coherence 协议传播（invalidate / update），保证最终一致

------

# 一句话总结

> CPU 保证“数据不会坏”，但不保证“你的并发逻辑是对的”。

# 什么是伪共享

可以把伪共享理解成一种**“被缓存误伤的并发性能问题”**，重点不在数据本身，而在**它们挤在同一块 cache line 里**。

------

# 一句话定义

> **不同线程操作的是不同变量，但因为这些变量落在同一个 cache line，导致彼此频繁让对方缓存失效，从而性能急剧下降。**

------

# 用一个通俗类比

想象一张纸（= cache line，通常 64 字节）：
...
1. 线程1拿到这张纸（cache line）
2. 线程2想改 `b` → 必须把线程1手里的纸抢过来
3. 线程1再想改 `a` → 又要抢回来
...
> 伪共享不是“你们用了同一个变量”，而是“CPU误以为你们在抢同一块缓存”，导致性能被拖垮。
